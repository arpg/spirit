...
double V(double th, double x, double y, double v, double p){
  //path following:
  return 0.251836191408*p + 2.31106217689*th*th + 3.23504654641*x*x + 3.28654452988*y*y + 1.77745630912*v*v - 1.03187637766*th*x - 0.521359195573*th*y - 0.161523642071*th*v + 0.825082067471*x*y - 0.914355165442*v*x + 0.0894413885605*v*y;
}

double V_pure(double th, double x, double y, double v){
  return 2.31106217689*th*th + 3.23504654641*x*x + 3.28654452988*y*y + 1.77745630912*v*v - 1.03187637766*th*x - 0.521359195573*th*y - 0.161523642071*th*v + 0.825082067471*x*y - 0.914355165442*v*x + 0.0894413885605*v*y;
}
...
double dV(double u_1, double u_2, double u_3, double th, double x, double y, double v, double p){
  ...
  double r = (2*2.31106217689*th - 1.03187637766*x - 0.521359195573*y - 0.161523642071*v)*(-2.9380*v_t*u_2 - dth_r*(1+u_3)) + 
    (2*3.23504654641*x - 1.03187637766*th + 0.825082067471*y - 0.914355165442*v)*(dth_r*(1+u_3)*y + v_t*-sin_th) + 
    (2*3.28654452988*y - 0.521359195573*th + 0.825082067471*x + 0.0894413885605*v)*(dth_r*(1+u_3)*(-x) + v_t*cos_th - v_r*(1+u_3)) + 
    (2*1.77745630912*v - 0.161523642071*th - 0.914355165442*x + 0.0894413885605*y)*(u_1);
  double boundary = 1;
  double L = V(th, x, y, v, p);
  if( L > boundary )
    r += 0.251836191408*(1+u_3);
  return r;
}
...
Input K(double th_t, double x_t, double y_t, double v_t, double p_t, int seg_prev, double u_1_prev, double u_2_prev, double u_3_prev){
  ...
  double b_1 = (2*1.77745630912*v - 0.161523642071*th - 0.914355165442*x + 0.0894413885605*y);
  double b_2 = (2*2.31106217689*th - 1.03187637766*x - 0.521359195573*y - 0.161523642071*v)*(-2.9380*v_t);
  double b_3 = (2*2.31106217689*th - 1.03187637766*x - 0.521359195573*y - 0.161523642071*v)*(-2.9380*v_t*0 - dth_r*(1+0)) + 
    (2*3.23504654641*x - 1.03187637766*th + 0.825082067471*y - 0.914355165442*v)*(dth_r*(1+0)*y) + 
    (2*3.28654452988*y - 0.521359195573*th + 0.825082067471*x + 0.0894413885605*v)*(dth_r*(1+0)*(-x) - v_r*(1+0)) + 
