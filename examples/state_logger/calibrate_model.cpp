...
/*
  ...
  for(int ii=0; ii<log.state_input_vec_.size()-1; ii++){
    ...
    time_diff = log.state_input_vec_[ii+1].timestamp - log.state_input_vec_[ii].timestamp;
...
*/
  ...
  ceres::CostFunction* calib_cost = new AnalyticalCalibCostFunc(log,num_params);
