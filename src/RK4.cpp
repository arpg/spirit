...
Eigen::MatrixXd RK4::Solve(const Eigen::VectorXd& init_cond, double horizon) {
...
void RK4::RK4Iteration(Eigen::VectorXd& x_t) {
  ...
  k1 = ode_func_(x_t);
  k2 = ode_func_(x_t+h_*(0.5*k1));
  k3 = ode_func_(x_t+h_*(0.5*k2));
  k4 = ode_func_(x_t+h_*k3);
  x_t = x_t+(h_/6.0)*(k1+2*k2+2*k3+k4);
}
