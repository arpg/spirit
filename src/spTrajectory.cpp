...
spObjectHandle spTrajectory::AddWaypoint(const spPose& pose) {
  spObjectHandle waypoint_handle = objects_.CreateWaypoint(pose,spColor(0, 0, 1));
  ...
  spObjectHandle linestrip_handle = objects_.CreateLineStrip(spPose::Identity(),line_pts,spColor(1, 0, 0));
