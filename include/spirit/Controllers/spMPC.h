...
template<typename simfunctor>
spMPC<simfunctor>::spMPC(const spVehicleConstructionInfo& car_params, float horizon_duration): car_params_(car_params) {
  horizon_ = (int)(horizon_duration/DISCRETIZATION_STEP_SIZE);
}

template<typename simfunctor>
spMPC<simfunctor>::~spMPC() {

}

template<typename simfunctor>
int spMPC<simfunctor>::CalculateControls(const spTrajectory& ref_traj, const spState& curr_state, spCtrlPts2ord_2dof& controls) {
...
template<typename simfunctor>
int spMPC<simfunctor>::CircleManReg(const spState& curr_state, spCtrlPts2ord_2dof& controls, double radius, double tangent_vel) {
...
template<typename simfunctor>
void spMPC<simfunctor>::SetHorizon(float horizon_duration) {
  horizon_ = (int)(horizon_duration/DISCRETIZATION_STEP_SIZE);
}
...
template<typename simfunctor>
void spMPC<simfunctor>::FindClosestTrajPoint(int& closest_waypoint_index, int& closest_subindex, const spTrajectory& ref_traj, const spState& curr_state) {
...
template<typename simfunctor>
void spMPC<simfunctor>::MinimizeMPCError(const spStateSeries& ref_states,const spState& current_state, spCtrlPts2ord_2dof& controls) {
...
template<typename simfunctor>
void spMPC<simfunctor>::MinimizeMPCError(const Maneuver& maneuver, const spState& current_state, spCtrlPts2ord_2dof& controls) {
  ceres::Problem problem;
  Eigen::VectorXd residual_weight(12);
