...
class CarODE : public ODEInterface {
public:
  CarODE(){
  }
  ...
  void Run(Eigen::VectorXd& x_d_t, const Eigen::VectorXd& x_t, const Eigen::VectorXd& u_t){
    ...
    double TireLon0Istate = x_t[3];
    double TireLon1Istate = x_t[4];
    double TireLon2Istate = x_t[5];
    double TireLon3Istate = x_t[6];
    double vx = x_t[0];
    double vy = x_t[1];
    // wrapping chi angle
  ...
  //  double TireLon3Rr = 2.2+(1/(1+20*(k3*k3)));
    double p0 = 25;
    double TireLon0Rr = p0+(0.2/(1+20*(k0*k0)));
    double TireLon1Rr = p0+(0.2/(1+20*(k1*k1)));
    double TireLon2Rr = p0+(0.2/(1+20*(k2*k2)));
    double TireLon3Rr = p0+(0.2/(1+20*(k3*k3)));
    ...
    double TireLat0Rr = p0+(0.2/(1+20*(bet0*bet0)));
    double TireLat1Rr = p0+(0.2/(1+20*(bet1*bet1)));
    double TireLat2Rr = p0+(0.2/(1+20*(bet2*bet2)));
    double TireLat3Rr = p0+(0.2/(1+20*(bet3*bet3)));
...
//    Eigen::VectorXd x_dot(x_t);
    ...
    x_d_t[7] = V*std::cos(chi+beta);
    // x_dot in inertial frame
    x_d_t[8] = V*std::sin(chi+beta);
