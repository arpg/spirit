...
template<typename simfunctor>
spLocalPlanner<simfunctor>::spLocalPlanner(const spVehicleConstructionInfo& vehicle_info, bool overwrite_endstate, Gui* gui):
...
template<typename simfunctor>
spLocalPlanner<simfunctor>::~spLocalPlanner() {
}

template<typename simfunctor>
void spLocalPlanner<simfunctor>::SetCostWeight(const spBVPWeightVec& vec) {
  weight_vec_ = vec;
}

template<typename simfunctor>
double spLocalPlanner<simfunctor>::SolveLocalPlan(spCtrlPts2ord_2dof& controls, double& simulation_duration, const spState& current_state, const spWaypoint& end_waypoint) {
...
template<typename simfunctor>
void spLocalPlanner<simfunctor>::SolveLocalPlan(spTrajectory& trajectory) {
...
template<typename simfunctor>
double spLocalPlanner<simfunctor>::SolveLocalPlan(spTrajectory& trajectory, int way_index) {
  int next_index;
...
template<typename simfunctor>
void spLocalPlanner<simfunctor>::SolveInitialPlan(spTrajectory& trajectory, int way_index) {
